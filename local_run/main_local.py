# local server that communicates with a local database for testing purposes
import os.path
...
import base64
import pickle
from database_local import get_student_data, enroll_student, authenticate
import sys
...
def encode_image(image, label):
    ...
    file_append = open('..\\db\\encodings.p', 'wb')
    pickle.dump([encoded_images_database, labels_database], file_append)
    file_append.close()
...
async def login(req: Request):
    data = await req.json()
    return authenticate(data['id'], data['password'])
